...
有趣的是，这台机器的全部零件我们都已见过。第六章的编码四环，倒过来就是解码的流水线；第五章的 PTS，正是播放节拍的裁判；第七章的队列与背压，将在播放器管线里再度重演。本章要做的就是把这些散落的零件装成整机，沿 "解码器 → 播放器 → 实战" 的主线：先解剖解码器框架的本质、分层与状态机，并走出 CPU 看看硬件解码的接口世界；再深入播放器核心的三个问题，看主时钟与抖动缓冲如何治理时间，顺带补齐字幕与 HDR 这两条进阶支线；最后用一场同步策略对比实验，让整章理论在数据里显影。

通过本章节的学习，读者将能够说清解码器框架的设计要素与 FFmpeg 六库的分工，讲透音画同步的主时钟机制与抖动缓冲的权衡，了解硬解接口、字幕渲染与 HDR 色调映射的工程要点，并能亲手搭建一台可复现的播放器仿真骨架。

>**关键字：解码器、FFmpeg、收发分离、PTS/DTS、硬件解码、音画同步、主时钟、抖动缓冲、渲染管线、字幕、HDR、色调映射**
...
* [8.1 解码器框架设计（Decoder Framework Design）](Docs_8_1.md)
	...
	* [8.1.5 硬件解码：平台接口与零拷贝（Hardware Decoding & Zero-Copy）](Docs_8_1_5.md)
* [8.2 播放器核心系统（Player Core Systems）](Docs_8_2.md)
	...
	* [8.2.5 字幕渲染：从文本到画面（Subtitle Rendering）](Docs_8_2_5.md)
	* [8.2.6 HDR 渲染：色调映射入门（HDR & Tone Mapping）](Docs_8_2_6.md)
