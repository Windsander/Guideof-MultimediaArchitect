...
它的思路很朴素：把帧的两端压到零，消除截断带来的突变，旁瓣衰减便可提升到约 -31.5dB。代价是 **主瓣（Main Lobe）变宽**——频率的分辨精度略有下降。这正是窗函数设计的永恒主题：**主瓣宽度与旁瓣衰减的权衡**。汉明窗（Hamming）、布莱克曼窗（Blackman）等经典窗函数，都是在这个权衡曲线上选取的不同工作点 [\[11\]][ref] 。
