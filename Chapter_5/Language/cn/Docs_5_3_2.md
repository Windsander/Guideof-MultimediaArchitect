...
需要强调的是，帧差法对 **硬切** 几乎百发百中，但对 **渐变转场** 与 **剧烈全局运动**（如快速摇镜）会失效——前者抬升平缓不越阈，后者整条曲线被抬高淹没切点。工业级场景切分还会结合 **颜色直方图距离**、**特征点匹配** 等多特征投票。但对入门工程而言，帧差 + 自适应阈值已经是一条可以跑通的完整链路。
...
**可见，帧间分析是一套从粗到细的工具体系：帧差量化整体变化，阈值检测定位结构边界，光流刻画逐像素运动。**

理论已经齐备。下一节，我们把这些工具组装成一条完整的视频帧分析流水线，并在完全可控的合成视频上验证每一个环节。
