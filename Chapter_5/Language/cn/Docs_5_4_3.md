...
**先验证音频链的每一项可计算的预期。** 时长：$13.00 / 0.75 = 17.33$ 秒，实测输出 17.34 秒（尾部采样点舍入），变速精确成立；变调只改音高不改时长，符合 Phase Vocoder 组合的设计。能量：低通后 RMS 从 0.0949 降至 0.0917（仅 -3.4%）——这是因为 A4 乐音的绝大部分能量集中在 2kHz 以下的基频与低次泛音，滤波削掉的主要是高频泛音与噪声；而变速变调后 RMS 降至 0.0347，这是相位声码器帧重排叠加的正常能量重分布（听感响度由归一化与后续增益环节负责）。
...
图 5-21 同时验证了三个环节：波形时间轴从 13s 拉长到 17.3s 而包络形态保持；梅尔频谱中，2kHz 以上的泛音亮带在 **处理后明显变暗但没有消失**——这正是 5.4.1 强调的 Butterworth 过渡带特性；同时各音带的频率位置整体上移约 $2^{3/12}$ 倍（变调的签名），而时间结构保持等比拉伸。
...
**再验证视频链。** 360 帧全程无丢帧地写出，耗时 0.59s，单帧成本 1.65ms——远低于 30fps 实时预算的 33.3ms，印证了 5.4.2 的预算讨论。
...
图 5-22 中，三个场景的处理帧呈现一致的变化：**背景色彩更浓郁**（场景 B 的暖红与方块色明显更艳，HSV 饱和度增益的效果）；**噪声颗粒被柔化**（高斯模糊）；**目标边缘保持清晰**（USM 把模糊损失的高频加了回来）。三步各有分工，叠加后得到 "鲜艳而干净" 的画面。处理后的完整视频见 **<a href="../../Examples/practice_6_video_processed.mp4" target="_blank">practice_6_video_processed.mp4</a>**。

**可见，两条管线共用同一副骨架：读入、逐单元加工、写出、报告——分析与处理，本就是同一套工程功法的两个方向。**

## **小结与延伸**

至此，第五章的完整闭环已经走完：**5.1** 备齐工具，**5.2** 分析音频帧，**5.3** 分析视频帧，**5.4** 处理两种帧。批量处理更多素材时，只需把输入替换为文件遍历；要做实时处理时，把数据源换成采集回调，并按 5.4.2 的预算方法评估帧函数成本即可。

配套的 **在线演示（见【在线展示】页）** 提供了帧处理的交互版本：音频区可以实时切换原声与低通滤波效果并调节截止频率；图像区可以拖动滑块组合饱和度、模糊与锐化，左右对比原图与处理结果。

下一节是全章小结：我们将把 "帧" 这条主线重新收拢，并由此眺望第六章——编码的世界。
