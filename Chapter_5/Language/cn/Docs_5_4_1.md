...
工程实现上还有一个细节值得记住：普通滤波会引入 **相位延迟**（不同频率延迟不同），离线处理时若要求波形无相位失真，可以使用 **前向-后向两次滤波（filtfilt）** 抵消相位偏移——practice_6 采用的便是这种方式。
...
所以，音频帧处理并不需要引入全新的概念：滤波负责塑造频率，相位声码器负责重塑时间，而二者的组合，便实现了音高的独立调整。
