...
首先对照 5.3.1 的时间体系。360 帧、30fps、12 秒与合成参数完全一致；第 120 帧的 PTS 恰为 4000ms，名义帧间隔 33.33ms 也与实测吻合。理论在工程接口中得到了印证。

然后是场景切分的精确性。自适应阈值为 0.006，而切点峰值达到 0.344，相差接近 60 倍。检出的 119、239 是 **切前最后一帧的 0 基索引**，换算后即真值 120、240，且 360 帧全程 **零误检、零漏检**。帧差法对硬切的可靠性，由此可以看得非常清楚。

光流数据则揭示了 "平均" 与 "极值" 的分工。两个帧对的全图平均幅度都只有 0.1 px/帧 量级——因为 98% 以上的像素是静止背景；而最大幅度（7.08 与 9.50 px/帧）才对应运动目标的真实速度。所以在分析运动时，**局部统计往往比全局均值更有信息量**。

最后是跟踪的结果。朴素质心法在三个场景上的轨迹图如下：
...
- **场景 A** 的轨迹是一条平滑的正弦曲线，与圆形目标的真实运动完全吻合——单目标、匀速、无遮挡时，朴素方法已经足够好用
- **场景 B** 的轨迹则明显凌乱：两个运动目标交替成为 "最大运动块"，质心在两者之间来回跳变。**"最大轮廓" 假设的固有缺陷** 在此暴露无遗——它无法表达多目标的情况
- **场景 C** 的轨迹整体是对角直线，但当方块冲出版面边缘并 **从另一侧回绕** 时，轨迹出现了长距离的瞬移连线。这暴露了朴素方法的另一个问题：**没有身份一致性**——目标消失再出现时，方法并不知道 "这是同一个它"

所以，朴素帧差跟踪在这里是一个很好的示例：它既展示了方法在什么条件下可用，也暴露了方法自身的边界。而要跨越这些边界——多目标、遮挡、重现识别——就需要 **[5.1.3](Docs_5_1_3.md)** 中介绍的 `cv2.Tracker` 族专业跟踪器，乃至第四章的检测与跟踪相结合的学习范式。

至此，一条百余行的流水线，完成了从像素到结构（切点）、从结构到运动（光流）、再从运动到目标（轨迹）的完整分析过程。

## **小结与延伸**

本节工程覆盖了视频帧分析的三个经典层次：结构分析（场景切分）、运动场分析（光流）、目标分析（跟踪）。需要分析真实视频时，只需跳过合成阶段、直接以文件路径初始化 `VideoCapture` 即可，其余环节可以原样复用。

**在线演示（见【在线展示】页）** 同样备好了视频分析的交互版本：在浏览器里打开任意本地视频，光流场会以 HSV 编码实时呈现运动矢量，拖动阈值滑块，即可观察运动强度曲线上切点检出的变化。
