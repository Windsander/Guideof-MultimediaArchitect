...
工程上，训练特征集 通常以 **一批次（1 Batch）** 样本计算后，由神经网络输出的当前权重下，输入样本的抽象非零解集构成。这个输出的抽象特征向量数据集，才是真正被我们用来衡量当前迭代结果情况的决定数据。即，**损失函数（Loss Function）作用的部分**。
...
经由神经网络抽象高维特征的输出向量数据集，被我们称为嵌入特征向量组（Embeddings of Low-Dimensional Features），简称嵌入集（Embeddings）。与特征工程的相关称谓同名，并不矛盾。
