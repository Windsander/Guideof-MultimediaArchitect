...
这即是神经网络雏形，**多层感知器（MLP [Multilayer Perceptron]）** 的灵感来源。
...
- 以 $$x$$ 表示一个输入的数值信号；
- 以 $$w$$ 表示当前输入的 **附加权重（weight）**，既可作为 **参与训练** 的层级特征权重，**也可为常数** ；
...
但是，**输出层（Output Layer）** 由于本身主要作用，是 **接收连接函数（Connection Function）计算预测值**，不会使用到 权重 $$w$$ 和 偏移 $$b$$ 。我们一般为了方便起见，会将作用于输出层与前一级隐藏层之间的连接函数，整合到输出层神经元中来便于代码实现。取链接函数为 $$f(x)$$ 表示，有：
