...
我们用它来做一个，基于 MNIST 手写字母图像集的，简单字母分类识别模型。
如下所示：
...
<figure>
   <img  
    ...
    <figcaption>
      <p>图 4-38 以 AlexNet 部署的 MNIST 字母识别模型</p>
   </figcaption>
</figure>
</center>
...
鉴于此，我们不得不采用 **功能分类** 方式，细化隐藏层类，来扩展对 CNN 的结构描述能力。
...
- **平化层（Flat [Flatten Layer]）**，处理多维数据向指定维度的平化展开工作；
...
**CNN 平化层（CNN-Flat）**，从字面意义理解，即把输入变换到指定维度大小数据的特殊处理层。它的意义在于，**为传统 MLP 的神经网络部分，提供可供其学习的输入特征**。
...
通常情况，我们会选择输出扁平化到 **一维张量（1-dim Tensor，即含有 $$n \times 1$$ 个元素的向量）**。这个过程如上图展示。
...
<figure>
   <img  
    ...
    <figcaption>
      <p>图 4-42 以 AlexNet 部署的 ImageNet 物体识别模型</p>
   </figcaption>
</figure>
</center>
...
- **特征选择子网（FS）**，在本例中根据功能也被称为 分类子网（Classification Subnet），包含接收平化层输出的相邻隐藏层至输出层前一级隐藏层。这些隐藏层都是全连接层，以此完成特征向量提炼。
