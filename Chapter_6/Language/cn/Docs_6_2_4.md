...
H.264 定义了 **52 个量化参数 QP（Quantization Parameter，0~51）**，量化步长随 QP 按对数增长：**QP 每增加 6，步长翻倍**。这样的设计让码率控制非常平滑——编码器微调一两个 QP，就能得到码率的精细变化，实践中常见的 CRF（恒定质量因子）模式正是建立在这把对数标尺上。
