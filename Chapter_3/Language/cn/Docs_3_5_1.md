...
即由空域离散化（SDD）与频域离散化（FDD）共同构成空频离散化（SFD [Spatial Frequency Discrete]）表达的 **二维离散傅立叶（2D-DFT）**，如下所示：
...
考虑到冗余压缩，并不要求保证数据帧完整不可分的输入，且精度也相对分析场景要求较低。如果能够适当的利用指数函数三角函数化，其本身的周期规律和标准化约束，建立基底整体的规律性，来契合傅立叶变换的性质。就能够在消减不必要参数（常量固定）并限定生效范围后，实现对离散傅立叶变换的常量化矩阵运算。建立卷积核，加速压缩过程。
...
<figure>
   <img  
    ...
    <figcaption>
      <p>图 3-22 示例样本目标 DST 补充后与切片索引关系图示</p>
   </figcaption>
</figure>
</center>

新的样本集，数据量较原有数据翻了一倍多。但只有 $$x$$ 轴正向的取值有意义。所以，采用 DST 类型分解，在扩充后，周期跨度都变为了 $$T= 2N + 1$$ ，且原离散展式 **只有 $$n \in [1, \ N]$$ 的部分是有效的**。我们可以将偏移的 $$+1 \times Step$$ 划到式中处理，则 $$n$$ 的取值范围就仍然可以保持为 $$n \in \mathbb{Z} [0, \ N - 1]$$ 。
...
当目标分解为 DCT 时，需要在基于 $$y=s(n)$$ 轴对称前，先行平移原数据 $$+\tfrac{1}{2} Step$$ 个步长。得到如下新集合（蓝色为补充数据，红色为原数据）：
...
<figure>
   <img  
    ...
    <figcaption>
      <p>图 3-23 示例样本目标 DCT 补充后与切片索引关系图示</p>
   </figcaption>
</figure>
</center>
...
{\displaystyle 
 \begin{aligned}
  ...
  DCT:& X_k  = \frac{1}{\sqrt{2N}}  \cdot \hat{s}(k) \quad \& \quad X_0  = \frac{2}{\sqrt{2N}}  \cdot \hat{s}(0) \\
 \end{aligned}
}
$$
...
这个即为 **整数正弦变换（IDST）核心公式**，而 $$\hat{K}_{DST}$$ 则被称为 **整数正弦变换的基本算子（IDST Opt）**。显然，在已知 $$S_p$$ 和存储范围 $$D$$ 的情况下，还是非常容易求得 $$X_k$$ 的。而对应的 GPU 程序片也很简单，基本可当作滑动窗口移动步长 $$K = 4$$ 的固定算子乘法运算，就不再复写了。

## **整数离散余弦变换（IDCT）的 GPU 矩阵算子**

同理于 IDST，虽然 **整数离散余弦变换（IDCT）** 的切入理论，和 IDST 有一些不同。但最终的算子区别仅在于取值上。

仍然需要，将离散余弦变换扩展到二维情况。有：
...
{\displaystyle 
 \begin{aligned}
  ...
  &\varepsilon_k|_{k=(0,0)} = \frac{1}{\sqrt{2}} \quad \& \quad \varepsilon_k|_{k \ne (0,0)}=1 \\
  DCT: X_k(u,v)  &= \begin{pmatrix} \frac{\varepsilon_k}{\sqrt{2}}  \end{pmatrix} ^2 \cdot \sum_{p = (0,0)}^{(3,3)}S_p(x,y) \cdot cos(\tfrac{2x+1}{8} \pi u) \cdot cos(\tfrac{2y+1}{8} \pi v) \\ 
 \end{aligned}
}
$$
...
这个即为 **整数余弦变换（IDCT）核心公式**，而 $$\hat{K}_{DCT}$$ 则被称为 **整数余弦变换的基本算子（IDCT Opt）**。同样，在已知 $$S_p$$ 和存储范围 $$D$$ 的情况下，还是非常容易求得 $$X_k$$ 的。而对应的 GPU 程序片也很简单，基本可当作滑动窗口移动步长 $$Step_K = 4$$ 的固定算子乘法运算，就不再复写了。
