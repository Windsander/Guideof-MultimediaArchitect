...
同时，由于 **包含高权重的中心值** 参与了计算过程。因此，拉普拉斯滤波 **并非是脱离中心参考值的边缘锐化（Edge Sharpening）算法** ，对于一些复杂的边缘波动情况，会产生 **边缘扩散（Edge Spread）** 的风险。而去中心化（Center Insensitive）可在一定程度上避免该情况问题。基于这种思路，我们有了 **索贝尔滤波（Sobel Filter）** 。
