...
**索贝尔滤波（Sobel Filter）** 是由 **斯坦福人工智能实验室（SAIL [Stanford Artificial Intelligence Laboratory]）** 的 **艾尔文·索贝尔（Irwin Sobel，1940 - present）** 和 **加里·费尔德曼（Gary Feldman，1942 - present）** 于 1968 年提出的一种用于 **边缘检测（Edge Detection）** 的 **去中心化（Center Insensitive）一阶离散微分算子** [\[15\]][ref] 。
...
显然，索贝尔滤波是同时具有 **梯度方向（Orientation）** 和 **强度（Magnitude）** 的。记方向为 $$\Theta$$ ，强度为 $$A$$ 。则有：
...
void main()
{
    ...
    vec3 color_center_x = vec3(0.0);
    vec3 color_center_y = vec3(0.0);
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
...
不过，这些缺点在只需要边缘位置的情况下，可以通过 **阈值限定二值化（Thresholding）** 来得到一定程度的改善（ **这种做法经常出现在机器学习的数据前处理过程中** ）。由于一般音视频工程并不会需要如此精度，考虑到索贝尔滤波的快捷、简单、高效和高干扰抗性的特点，算法本身常被用于各种场景下的 **边缘数据提取** 和 **像素信息预测** 过程。但本身不适合(也不应该)作为噪声抑制算法使用。

经过几个滤波算法的辨析，我们发现想要真正的有效抑制噪声，达到自然模糊且边缘保存的目的，单纯以多 **非各向异性** 滤波器组合的形式，还是很难得到同 **各向异性** 滤波算法相同的效果。
