...
根据原论文的设计，IoU 前向传播作用在 ReLU 激活函数层（ReLU Layer）后，以代替传统物体识别模型采用的 $$L_2$$ 损失函数，判断待筛选的预测框是否命中。由于始终有 $$IoU \in [0, \ 1]$$ ，交并比损失函数可被认为是 $$p(IoU = 1) = 1$$ 的 **特殊交叉熵损失函数（cross-entropy Loss）**，有：
...
带入求得 $$IoU\ \mathcal{B}ack$$ 值，作用于 **优化算法的梯度变换**，如 **自适应实时评估算法（Adam）** 等。来发挥相应作用。
...
<figure>
   <img  
    ...
    <figcaption>
      <p>图 3-12 红框即为 GIoU 图例中，I 和 U 的最小外接矩形</p>
   </figcaption>
</figure>
</center>
