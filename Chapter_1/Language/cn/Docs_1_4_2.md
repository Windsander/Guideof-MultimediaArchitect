...
**和声（Harmony）** 是指将音调以两两形式组合，而产生新的声音的过程。当然，为了给予艺术发挥空间，不会也不能固定响度入内。所以，和声是指参与声音其音色频率链的合成。此处为了通用说明，需要固定图形化时，采用相同大小的抽象响度表示。

不过请注意，在实际数字合成过程中，还是需要 **结合响度构建和声后的新谐波链的**。
...
但相仿 P1 + P5 和声情况的音调组合，其 **谐波链存在较多重合，却依然可以被分辨**。我们称其为 **不完美协和（Imperfect Consonance）**。
...
在工程上，相对较常用的是三度弦。而三度弦分类下，各种和弦概念存在基本规律，可以直接从三和弦向上衍生。因此，为了便于记忆，本书采用三和弦（三音）讲解。至于非三度弦的其它类型，借助对三和弦的理解，需要时再行查阅乐理专业资料即可。
...
**现代调性网络（Tonnetz）原型**，来自于数学大家 **莱昂哈德·欧拉（Leonhard Euler，1707～1783）** 在早年尝试的，以数学建模构造良好和声的探索 [\[10\]][ref] 。欧拉首次采用数学图论方法，解决和弦问题，提出了 **欧拉调性网络**（Euler's Tonnetz，Tonnetz 是德语，相当于英文的 Tone-net）。
...
状态一直持续到 19 世纪末，被 **胡戈·里曼（Hugo Riemann，1849～1919）** 打破。
