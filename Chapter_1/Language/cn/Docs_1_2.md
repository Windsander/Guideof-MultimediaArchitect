...
**声音（Sounds）** 是对所有由振动产生，可以被人感知并理解的，由固体、液体、气体等介质而传播的一类 **声波（Acoustic Wave）** 的统称 [\[1\]][ref] 。只有被人能够听到的声波，才属于声音。所以，声音也可以被称为狭义声波（Narrow Acoustic Wave）。由于其本质仍是声波。客观对声波的测定量，也是同样作用于声音的。
...
既然是机械波，那么从波的传播角度，就可以根据机械波的物理性，测量出衡量声波的一维传播方向关系：
...
- 以 $$x$$ 代表 **声位（Spatial Position）**，即声波的当前空间位置，单位常用 米（m）
- 以 $$t$$ 代表 时刻，单位常用 秒（s）

这就是著名的 **一维声波方程（1D Acoustic Wave Equation）**。而以 $$\vec{x}$$ 表示当前声位在空间中距离发出点（即原点，我们假设声波从原点产生）的位置，将传播从一维扩展到 $$dim(\vec{x}) = n$$ 维空间，则有：
...
得到了通用的 **理想声波方程（Ideal Acoustic Wave Equation）**，也称为 **费曼三维声波方程（Feynman's 3D Acoustic Wave Equation）** [\[3\]][ref] 。
