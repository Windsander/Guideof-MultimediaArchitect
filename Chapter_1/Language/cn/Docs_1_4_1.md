...
图中，**大写字母代表自然大调（Major）** ， **小写字母代表自然小调（Minor）** 。以此为出发点，转换到同音级处理。就有， **同圈层** 的音调，相邻两个音调间的音程（Interval），顺时针时差值为 $$\Delta n = ({n_{tag} - n_{com}}) = 7 \rightarrow k=5$$ 纯五度（P5），逆时针时为 $$\Delta n = ({n_{tag} - n_{com}}) = 5 \rightarrow k=4$$ 纯四度（P4）。

在音乐（艺术）中， **调号（Key Signatures）** 指 **音调（Note）所带的升降号（ $$\sharp/\flat$$ ）** 。

**关系调（Relative Key）** 的定义便是调号（Key Signatures）相同而主音不同。有互为关系调的大小调，两者间的 **主音（Keytone）** 音程为 **小三度（m3）** 。显然，五度圈中位于 **同位置内外圈** 的大小调，就是这样的关系。
