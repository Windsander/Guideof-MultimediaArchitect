...
明明是 八度音，却有包括 C5 在内的 13 个音调。尺度不一太尴尬了，怎么办呢？
...
所以，除 $$\Delta n = 6$$ 情况外，我们一般只用它来向高频方向计数。而这种处理使得以 **n.AI 公式计算出来是多少 k ，就应该称为增减多少 k 度（AK/dK）**。
...
所以，在跨级描述上，相对音程情况需要引入其它的量词用以记录级数差。一个简单的方法就是 **在公式基础上，根据跨越的级数，在称呼中增加 级数 x 七度 的大小**。
...
例如，C 大调（C Major）的主音（Keytone）就是 C4 261.63 Hz 。但总共包含：
...
此举启发了人们对古典五律的划分，从而有了 **自然大调（Major Scale）**，即 C大调， **的五度圈（Circle of Fifths）查寻法**。这是一种将上文 12 音调以圆圈的形式串联的表示方式。当然，人们创造出该方法的时候，是凭借着历史经验总结而来的。不得不令人感叹其中的智慧。
...
五度圈中一圈（不升降），就是音级为 4 时 ISO 16 标准的 A440 八度音阶 C4 子表。
...
例如，
- 对于五度圈更上一层的 $$C\sharp \sharp = C5 = 523.25 (+12)$$ ，而 $$C\sharp \sharp \sharp = D \flat \sharp = C\sharp 5 = 554.37 (+13)$$ 。
...
ISN 本身在乐理（艺术）上，是人为的尺度平均的。不过，乐理上的平均，是否意味着实际频率的平均呢？结合 A440 频率音序公式（FSF）判断可知， **乐理平均并不意味着均匀的频率划分**。这和人耳的听感息息相关。
