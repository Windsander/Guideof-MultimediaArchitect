...
\newcounter{x} \newcounter{y}
{
    ...
    \foreach \y in {0,...,7}
        \node[n, pin={[pin edge={latex'-,black}]left:$f(\y)$}]
              (N-0-\y) at (0,-\y) {};
    % Draw outputs
    \foreach \y / \idx in {0/0, 1/1, 2/2, 3/3, 4/0+4, 5/1+4, 6/2+4, 7/3+4}
        \node[n, pin={[pin edge={-latex',black}]right:$\hat{f}(\idx)$}]
              (N-3-\y) at (2,-\y) {};
   % draw connector nodes
    \foreach \y in {0,...,7}
        \foreach \x / \c in {1/1, 2/3}
    ...
    \setcounter{y}{0}
    \foreach \i / \j / \p in {0/4/0, 1/5/0, 2/6/0, 3/7/0, 4/0/1, 5/1/1, 6/2/1, 7/3/1} 
    {
        \path (N-2-\arabic{y}) edge[-] node {$DFT^{\i\cdot\j}$}
              (N-3-\arabic{y});
        \addtocounter{y}{1}
    }
    % Connect nodes
